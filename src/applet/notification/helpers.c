...
bool build_notification(cJSON **jroot, const char *eid, const uint32_t seqNumber,
...
bool parse_notification(const cJSON *jroot, const char *eid, uint32_t *seqNumber,
    ...
    if (!cJSON_IsString(jvalue)) return false;
    value = cJSON_GetStringValue(jvalue);
    if (strcmp(value, "notification") != 0) return false;

    jvalue = cJSON_GetObjectItem(jroot, "eid");
    if (!cJSON_IsString(jvalue)) return false;
    value = cJSON_GetStringValue(jvalue);
    if (strcmp(value, eid) != 0) return false;

    jvalue = cJSON_GetObjectItem(jroot, "seqNumber");
    if (!cJSON_IsNumber(jvalue)) return false;
